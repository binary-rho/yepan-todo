...
export function ProjectNotesPanel({
  ...
  function submit() {
    if (!body.trim()) return;
    if (!currentUser) {
      setError("좌측에서 내 정보를 먼저 입력해주세요.");
      return;
    }
    setError(null);
    startTransition(async () => {
      const result = await addProjectNote(
        { projectId, body: body.trim() },
        currentUser.id,
      );
      if (!result.ok) {
    ...
    <aside className="w-80 shrink-0 bg-white border-l border-zinc-200 flex flex-col h-full">
      <div className="px-4 py-3 border-b border-zinc-200 shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          {notes.length > 0 && (
            <span className="text-[12px] text-zinc-400 tabular-nums tracking-tight">
              {notes.length}
            </span>
          )}
        </div>
        <p className="text-[11px] text-zinc-400 tracking-tight">
          공유가 필요한 맥락·QA 이슈를 자유롭게 기록해 두세요.
        </p>
      </div>
      ...
      {!readOnly && (
        <div className="px-4 py-3 border-b border-zinc-100 shrink-0">
          ...
          />
          {error && (
            <p className="text-[11px] text-red-500 mt-1 tracking-tight">
              {error}
            </p>
          )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {notes.length === 0 && (
          <p className="text-[12px] text-zinc-400 tracking-tight text-center py-8">
            기록된 메모가 없습니다.
          </p>
        )}
        {notes.map((note) => (
          <div
            key={note.id}
            className="border border-zinc-200 rounded p-3 group"
          >
            <p className="text-[13px] text-zinc-700 tracking-tight leading-relaxed whitespace-pre-wrap">
              {note.body}
            </p>
