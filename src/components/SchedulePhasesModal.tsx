...
export function SchedulePhasesModal({
  phases,
  onClose,
  onSaved,
}: SchedulePhasesModalProps) {
      ...
      >
        ...
        <div className="p-4 space-y-3">
          ...
          <p className="text-[12px] text-zinc-500 tracking-tight">
            출시 국면과 기간을 등록합니다. 항목 생성 시 국면을 고르면 그{" "}
            <span className="font-medium text-zinc-700">시작일</span>이 마감일로
            자동 입력됩니다.
          </p>
          ...
          {rows.map((row, index) => (
            <div
              key={index}
              className="grid grid-cols-[1fr_130px_130px_auto] gap-2 items-center"
            >
              <input
