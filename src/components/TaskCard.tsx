...
export function TaskCard({
  ...
  const interactive = !readOnly && Boolean(onMove || onAssigneeChange || canNotify)
          ...
          <div className="mt-2.5 pt-2.5 border-t border-zinc-100 space-y-2" onClick={stop}>
            ...
            <div className="flex items-center gap-1 flex-wrap">
              ...
              {canNotify && (
                ...
                >
                  {notifyState === 'sent' ? <Check size={11} className="text-emerald-600" /> : <Bell size={11} />}
                  {notifyState === 'sent' ? '전송됨' : '호출'}
                </button>
              )}
            </div>
          </div>
