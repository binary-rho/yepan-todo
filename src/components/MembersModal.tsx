...
function TeamRoleSelect({
  ...
  value: TeamRoleOrUnset;
  onChange: (v: TeamRoleOrUnset) => void;
}) {
  return (
    <select
...
export function MembersModal({
  ...
  function toggleImported(email: string) {
    setImportedMembers(
      (prev) =>
        prev?.map((m) =>
          m.email === email ? { ...m, selected: !m.selected } : m,
        ) ?? null,
    );
  }
  ...
  function addSelectedImported() {
    ...
    startAddingImported(async () => {
      const result = await createMembersBulk(
        projectId,
        toAdd.map(({ name, email }) => ({ name, email })),
      );
      if (!result.ok) {
        setImportError(result.error);
        return;
      ...
      >
        ...
        <div className="p-4 space-y-3">
          ...
          {importedMembers && (
            <div className="border border-zinc-200 rounded p-3 space-y-2">
              <p className="text-[12px] font-medium text-zinc-600 tracking-tight">
                팀즈에서 {importedMembers.length}명을 찾았습니다. 추가할 멤버를
                선택해주세요.
              </p>
                ...
                {importedMembers.map((m) => (
                  <label
                    ...
                    className={`flex items-center gap-2 px-2 py-1.5 rounded text-[13px] tracking-tight ${
                      m.alreadyExists
                        ? "text-zinc-300"
                        : "text-zinc-700 cursor-pointer hover:bg-zinc-50"
                    }`}
                  >
                    <input
                      type="checkbox"
                    ...
                    {m.alreadyExists && (
                      <span className="text-[11px] shrink-0">이미 등록됨</span>
                    )}
                  </label>
                ))}
              </div>
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  disabled={
                    isAddingImported ||
                    importedMembers.every((m) => !m.selected || m.alreadyExists)
                  }
                >
                  선택 추가
                </button>
            ...
            {members.map((member) => {
              ...
              if (isEditing) {
                return (
                  <div
                    key={member.id}
                    className="flex items-center gap-2 p-2 border border-zinc-300 rounded"
                  >
                    <input
              ...
              return (
                <div
                  key={member.id}
                  className="flex items-center gap-2 p-2 border border-zinc-100 rounded hover:bg-zinc-50"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[13px] font-medium text-zinc-900 tracking-tight">
                        {member.name}
                      </span>
                      <span className="text-[11px] text-zinc-400 tracking-tight">
                        {member.teamRole ?? "미정"}
                      </span>
                    </div>
                    <p className="text-[12px] text-zinc-500 tracking-tight truncate">
                      {member.email}
                    </p>
                  </div>
                  <button
          ...
          <div className="pt-3 border-t border-zinc-100">
            <p className="text-[12px] font-medium text-zinc-600 tracking-tight mb-2">
              멤버 추가
            </p>
