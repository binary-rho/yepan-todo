...
- `/templates` 템플릿 관리(생성/수정/삭제) 및 템플릿으로 항목 일괄 생성
...
- **템플릿과의 관계**: 템플릿은 여러 회차에 재사용되므로 항목의 "기본 담당자"는 실제 배정이 아니라 자유 텍스트 힌트다. 템플릿을 적용해 항목을 생성하는 시점에 지금 회차의 실제 멤버 중에서 항목별 담당자를 고른다. 보드 상단 "템플릿" 버튼을 누르면 템플릿 관리 화면으로 이동하지 않고, 저장된 템플릿을 고르는 팝업이 떠서 곧바로 이 회차에 항목을 만들 수 있다.
