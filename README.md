...
로그인이 없다. 진입 즉시 보드가 보이며, 좌측 내비 하단 "현재 사용자"에 본인 이름·이메일을 직접 입력해 쓴다(브라우저에 기억되고 회차를 옮겨도 유지된다). 관리자/일반 등급 구분 없이 모든 멤버가 동등하다.

- `/` 보드: 3개 상태(할 일 / 완료 / 반려) 칸반, 진척률, 필터, 항목 생성, 일정, 템플릿으로 생성, 멤버 관리
- `/tasks/[id]` 항목 상세: 상태 전환, 코멘트, 이력, 스크린샷 첨부
- `/templates` 템플릿 관리(생성/수정/삭제). 템플릿은 대시보드에 귀속되지 않는 공용 자산이라 이 화면에는 회차/멤버 개념이 없다
- `/archive` 보관된 회차 목록(읽기 전용) 및 삭제

세팅 환경은 **STG / PROD** 두 개다(DEV 는 쓰지 않는다). 환경을 늘리거나 줄일 때는 `ENV_CONFIG`·`ENVIRONMENTS`(`src/lib/constants.ts`)와 DB enum(`environment`)을 함께 고친다.

### 템플릿 항목의 마감일

템플릿은 여러 회차에 재사용되므로 항목마다 마감일을 정하는 **방식**을 저장한다(템플릿 편집기의 "마감일" 줄).

| 방식 | 저장 내용 | 적용 시 마감일 |
| --- | --- | --- |
| 적용 시 기준일(기본) | 없음 | 템플릿 적용 팝업에서 입력한 기준 마감일 |
| 일정 기준 | 일정 이름 + 며칠 전/후 | 그 일정의 **시작일** ± 일수 |
| 날짜 직접 선택 | 고정 날짜 | 저장된 그 날짜 |

일정 목록은 보드 상단 "일정"에서 등록한 국면(사전예약, 출시 등)이다. 일정은 회차마다 다시 등록되며 그때 id 가 새로 발급되므로 템플릿은 id 가 아니라 **이름**으로 참조한다. 같은 이름의 일정이 없으면(이름을 바꿨거나 아직 안 등록한 경우) 그 항목은 기준 마감일로 대체되며, 템플릿 목록과 편집기에 "현재 일정에 없음"으로 표시된다.
...
- **담당자 추가/수정/삭제**: 보드/템플릿/항목 상세 화면 상단 "멤버" 버튼에서 지금 보고 있는 회차의 이름·이메일·직군(미정/사업/기획/TPM/FE/BE)을 관리한다. 이메일은 Teams 알림 @멘션 id 로도 쓰이므로 실제 조직 이메일을 넣는다.
- **현재 사용자**: 좌측 내비 하단에서 **본인 이름·이메일을 직접 입력**한다(브라우저 로컬에 저장, 실제 인증 아님). 멤버 목록에서 고르는 방식이 아니라 직접 입력인 이유는, 남의 이름을 한 번의 클릭으로 고를 수 있으면 잘못된 귀속이 너무 쉽게 일어나기 때문이다. 입력 전에는 항목 생성·상태 변경·코멘트 등 기록이 남는 동작이 모두 막힌다.
- **회차 멤버와의 연결**: 멤버 행은 회차마다 별개(같은 사람도 회차마다 다른 id)이므로, 입력한 **이메일**로 지금 보고 있는 회차의 멤버와 연결한다. 그 회차에 같은 이메일의 멤버가 없으면 화면 상단에 "이 대시보드의 멤버가 아닙니다" 알림이 뜨고, 거기서 바로 "이 대시보드 멤버로 추가"할 수 있다.
- **Teams에서 가져오기**: 같은 화면의 버튼으로 Microsoft Graph 연동을 시도한다(환경변수 미설정 시 실패 메시지만 뜨고 나머지 기능엔 영향 없음).
- **템플릿과의 관계**: 템플릿 자체는 대시보드에 귀속되지 않는 공용 자산이다. 여러 회차에 재사용되므로 항목의 "기본 담당자"는 실제 배정이 아니라 자유 텍스트 힌트이며, 실제 배정은 적용 시점에 그 회차 멤버 중에서 고른다. 보드 상단 "템플릿" 버튼을 누르면 저장된 템플릿을 고르는 팝업이 떠서 곧바로 이 회차에 항목을 만들 수 있다(템플릿 관리 화면으로 이동하지 않는다).
- **담당자 미지정**: 담당자는 필수가 아니다. "이 세팅이 필요하다"만 먼저 정해졌다면 담당자를 비워 등록하고(보드 카드/상세에서 언제든 지정), 필터의 "담당자 미지정"으로 배정 대기 목록을 모아 볼 수 있다. 담당자가 없는 항목에서 "알림"을 누르면 사람 태그 없이 채널에만 발송된다(담당 지정 요청 성격). 배정 알림과 크론 다이제스트는 알릴 대상이 없으므로 보내지 않는다.
...
- **즉시 발송**: 항목 생성/담당자 배정(담당자에게), `rejected`(담당자에게, 사유 포함). 담당자가 없는 항목은 배정 알림을 보내지 않고, 보드/상세의 "알림" 버튼으로만 사람 태그 없이 채널에 알린다.
