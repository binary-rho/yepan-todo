...
상태가 많아 보기 어렵다는 피드백에 따라 화면 상태를 3개로 줄였다. `in_progress`·`review_requested`(완료요청/승인 플로우)는 제거하고, 기존 데이터는 마이그레이션 0005 에서 '할 일'로 이관한다. enum 값 자체는 과거 이력 표시를 위해 남겨둔다. 상태 전환은 할 일↔완료/반려, 완료·반려→할 일(되돌리기)이며 반려 시 사유가 필수다. 라우트는 루트 `/` 를 보드로 통합했다(담당자/관리자 화면 분리 제거).
